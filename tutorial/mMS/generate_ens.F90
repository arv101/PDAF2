...
program ens_prog
	...
	stat(j) = NF90_INQ_DIMID(file_id_in, 'dim_state', dim_id)
  	...
  	! define dimensions
  	ens_size = 10				! hardcoded for now but needs to be made dynamic (sb 3/7) -- never made it dynamic lol (sb 9/7/23) 
  	j = j + 1
	...
	stat(j) = NF90_ENDDEF(file_id_out)
  	...
  	cnt = 1
  	loop: do iter = 2, nsteps-2, nsteps/ens_size
 		! read state vector
		start_v(2) = iter
		count_v(2) = 1
		start_v(1) = 1
